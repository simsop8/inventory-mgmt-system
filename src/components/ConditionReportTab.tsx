...
export const ConditionReportTab: React.FC = () => {
  ...
  const generateConditionReportPDF = async () => {
    ...
    try {
      ...
      const filename = `${(profile.details.address || 'property').replace(/[^a-z0-9]/gi, '_').slice(0, 50)}-condition-report.pdf`;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 bg-blue-50 border border-blue-200 rounded-lg p-4">
        ...
        <button
          ...
          className="shrink-0 flex items-center justify-center gap-2 px-4 py-2 text-sm font-medium text-white bg-primary-600 rounded-lg hover:bg-primary-700 transition-colors shadow-sm"
        >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3 bg-white rounded-lg shadow p-4">
        <div className="flex items-center gap-2">
          <label htmlFor="photos-per-page" className="text-sm font-medium text-gray-700 whitespace-nowrap">Layout</label>
          <select
            id="photos-per-page"
            value={photosPerPage}
            onChange={e => setPhotosPerPage(Number(e.target.value) as 2 | 4 | 6 | 8)}
            className="border border-gray-300 rounded-md px-2.5 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500"
          >
        ...
        >
          ...
          <div className="text-left">
            <div className="font-semibold text-primary-700 text-sm">{generating ? 'Generating…' : 'Preview Condition Report PDF'}</div>
            <div className="text-xs text-primary-500">Grouped by room — separate from the Report &amp; Takeover PDFs</div>
          </div>
        </button>
      </div>

      {profile.photos.length === 0 ? (
        <div className="bg-white rounded-lg shadow p-8 text-center text-gray-400 text-sm">No photos yet — tap "Add Photo" above to add your first one.</div>
      ) : (
        <div className="space-y-4">
          <div className="flex items-center justify-between bg-white rounded-lg shadow px-4 py-2.5">
            ...
            <div className="flex items-center gap-3">
              <span className="text-xs text-gray-400">{selectedPhotoIds.size} selected</span>
              <button
              ...
              <button
                ...
                disabled={selectedPhotoIds.size === 0}
                className="px-3 py-1.5 text-xs font-medium text-red-600 bg-red-50 rounded-md hover:bg-red-100 disabled:opacity-40 disabled:cursor-not-allowed"
              >
                Delete Selected
              </button>
            </div>
            ...
            return (
              <div key={area} className="bg-white rounded-lg shadow overflow-hidden">
                <div className="flex items-center gap-2 px-4 py-3 bg-gray-50 border-b border-gray-200">
                  ...
                  <span className="text-xs text-gray-400 bg-gray-200 px-2 py-0.5 rounded-full">{areaPhotos.length} photo{areaPhotos.length !== 1 ? 's' : ''}</span>
                </div>
                <div className="p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {areaPhotos.map(photo => (
                    <div key={photo.id} className={`border rounded-lg overflow-hidden ${selectedPhotoIds.has(photo.id) ? 'border-primary-400 ring-2 ring-primary-200' : 'border-gray-100'}`}>
                      ...
                      <div className="p-2.5 space-y-2">
                        <select
                          ...
                          className="w-full border border-gray-200 rounded px-2 py-1 text-xs bg-gray-50 focus:outline-none focus:ring-1 focus:ring-primary-500"
                        >
                        ...
                        <textarea
                          ...
                          rows={2}
                          className="w-full border border-gray-200 rounded px-2 py-1 text-xs focus:outline-none focus:ring-1 focus:ring-primary-500 resize-y"
                        />
                        <p className="text-xs text-gray-400">{new Date(photo.dateAdded).toLocaleString()}</p>
                        <div className="flex gap-2">
                          <button onClick={() => handleDownload(photo)} className="flex-1 text-xs px-2 py-1 text-gray-600 bg-gray-100 rounded hover:bg-gray-200">Download</button>
                          <button onClick={() => { if (confirm('Delete this photo?')) deletePhoto(photo.id); }} className="flex-1 text-xs px-2 py-1 text-red-600 bg-red-50 rounded hover:bg-red-100">Delete</button>
                        </div>
                      </div>
                    </div>
                  ))}
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
          ...
          <div className="relative w-full sm:max-w-md sm:mx-4 bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl p-4 pb-6 sm:pb-4 max-h-[85vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => setAddMenuOpen(false)} aria-label="Close" className="w-8 h-8 flex items-center justify-center text-gray-400 hover:text-gray-600 text-xl leading-none">✕</button>
            </div>
            <div className="mb-3">
              <label className="block text-sm font-medium text-gray-700 mb-1">Current Area</label>
              <select
              ...
              {addingCustomArea && (
                <div className="flex gap-2 mt-2">
                  ...
                  />
                  <button onClick={confirmCustomArea} disabled={!customAreaInput.trim()} className="px-3 py-2 text-sm font-medium text-white bg-primary-600 rounded-md hover:bg-primary-700 disabled:opacity-40">Use</button>
                </div>
              )}
              <p className="text-xs text-gray-400 mt-1">Photos you take go into this area. Switch it any time to move on to the next room — or add your own (e.g. "Others" for keys/remotes, or a custom name).</p>
            </div>
            <div className="flex flex-col sm:flex-row gap-3">
              <label className="flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-primary-50 text-primary-700 rounded-lg border-2 border-dashed border-primary-200 cursor-pointer hover:bg-primary-100 transition-colors">
                <span className="text-xl">🖼️</span><span className="font-medium text-sm">Choose from Library</span>
              ...
              <label className="flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-green-50 text-green-700 rounded-lg border-2 border-dashed border-green-200 cursor-pointer hover:bg-green-100 transition-colors">
                <span className="text-xl">📷</span><span className="font-medium text-sm">Take a Photo</span>
                <input ref={camRef} type="file" accept="image/*" capture="environment" className="hidden" onChange={e => processFiles(e.target.files)} />
              </label>
            </div>
            <p className="text-xs text-gray-400 mt-2">Snap, then keep tapping "Take a Photo" again to add more to the same area — close this sheet whenever you're done.</p>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex flex-col bg-black/60">
          <div className="bg-white px-4 py-3 shadow">
            <div className="flex items-center justify-between gap-2">
              <span className="font-semibold text-gray-900 text-sm truncate">Condition Report Preview</span>
              <div className="flex items-center gap-2 shrink-0">
                <button onClick={downloadPreview} className="px-3 py-1.5 text-sm font-medium text-white bg-primary-600 rounded-md hover:bg-primary-700">Download / Share</button>
                <button onClick={closePreview} aria-label="Close preview" className="w-8 h-8 flex items-center justify-center text-gray-400 hover:text-gray-600 text-xl leading-none">✕</button>
              </div>
            </div>
