    ...
    return (
      <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
        <div className="flex items-center justify-between mb-1">
          <span className="text-xs font-semibold text-yellow-800">Edit Item Template — {roomType}</span>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600 text-lg leading-none">×</button>
        </div>
        <p className="text-xs text-yellow-700 mb-2">Renames apply globally to all existing rooms. New items only apply when adding future rooms.</p>
        {renames.length > 0 && (
        ...
        <div className="space-y-1 max-h-48 overflow-y-auto mb-2">
          {list.map((item, i) => (
            <div key={i} className="flex items-center gap-1">
              ...
              <button onClick={() => startEdit(i)} className="text-gray-300 hover:text-primary-500 text-xs px-1" title="Rename (applies globally)">✎</button>
              <button onClick={() => remove(i)} className="text-gray-300 hover:text-red-500 text-xs px-1" title="Remove">×</button>
            </div>
          ))}
        </div>
        <div className="flex gap-1 mb-2">
          <input type="text" value={newItem} onChange={e => setNewItem(e.target.value)}
            ...
            placeholder="Add item to template..." className="flex-1 border border-gray-300 rounded px-2 py-1 text-xs focus:outline-none focus:ring-1 focus:ring-primary-500" />
        ...
        <div className="flex gap-1">
          <button onClick={() => { onSave(list, renames); onClose(); }} className="flex-1 py-1.5 text-xs text-white bg-primary-600 rounded hover:bg-primary-700">Save Template</button>
  ...
  return (
    <div className="flex items-center gap-2 p-2 bg-white border border-gray-100 rounded-lg">
      <div className="w-36 flex-shrink-0">
        {editingName ? (
          <input autoFocus type="text" value={nameVal} onChange={e => setNameVal(e.target.value)}
            ...
            className="w-full border border-primary-400 rounded px-2 py-1 text-xs focus:outline-none" />
        ) : (
          <button onClick={() => setEditingName(true)} title="Click to rename" className="text-sm font-medium text-gray-800 hover:text-primary-600 w-full text-left truncate">{item.name}</button>
        )}
      </div>
      <input type="text" value={item.brandModel || ''} onChange={e => onUpdate({ brandModel: e.target.value })} placeholder="Brand / Model"
        className="flex-1 border border-gray-200 rounded px-2 py-1 text-xs focus:outline-none focus:ring-1 focus:ring-primary-500 min-w-0" />
      <input type="text" inputMode="numeric"
        ...
        placeholder="Qty" className="w-14 border border-gray-200 rounded px-2 py-1 text-xs text-center focus:outline-none focus:ring-1 focus:ring-primary-500 flex-shrink-0" />
    ...
    return (
      <div className="mt-2 p-3 bg-blue-50 border border-blue-200 rounded-lg">
        <div className="flex gap-4 mb-2">
          <label className="flex items-center gap-1 text-xs text-gray-600 cursor-pointer"><input type="radio" checked={!isCustom} onChange={() => setIsCustom(false)} /> From list</label>
          <label className="flex items-center gap-1 text-xs text-gray-600 cursor-pointer"><input type="radio" checked={isCustom} onChange={() => setIsCustom(true)} /> Custom</label>
        </div>
        <div className="flex gap-2">
          {isCustom ? (
            <input autoFocus type="text" value={custom} onChange={e => setCustom(e.target.value)}
              onKeyDown={e => { if (e.key === 'Enter') handleAdd(); if (e.key === 'Escape') onClose(); }}
              placeholder="Enter item name" className="flex-1 border border-gray-300 rounded px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500" />
          ) : (
            <select value={selected} onChange={e => setSelected(e.target.value)} className="flex-1 border border-gray-300 rounded px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500">
              <option value="">Select item...</option>
              {available.map(n => <option key={n} value={n}>{n}</option>)}
            </select>
          )}
          <button onClick={handleAdd} disabled={isCustom ? !custom.trim() : !selected} className="px-3 py-1.5 text-sm text-white bg-primary-600 rounded hover:bg-primary-700 disabled:opacity-40">Add</button>
          <button onClick={onClose} className="px-3 py-1.5 text-sm text-gray-500 bg-white border border-gray-300 rounded hover:bg-gray-50">Cancel</button>
        </div>
      </div>
    );
  };
...
export const RoomsInventory: React.FC = () => {
  const { profile, isLocked, addRoom, deleteRoom, updateRoom, addItem, updateItem, deleteItem, reorderRoom, setRoomItemList, renameGlobalItem } = useProperty();
    ...
    <fieldset disabled={isLocked} className="space-y-4 m-0 p-0 border-0 min-w-0">
      <div className="bg-white rounded-lg shadow p-4">
        ...
        <div className="flex gap-3 mb-2">
          <label className="flex items-center gap-1 text-sm text-gray-600 cursor-pointer"><input type="radio" checked={!isCustomRoom} onChange={() => setIsCustomRoom(false)} /> From list</label>
          <label className="flex items-center gap-1 text-sm text-gray-600 cursor-pointer"><input type="radio" checked={isCustomRoom} onChange={() => setIsCustomRoom(true)} /> Custom name</label>
        </div>
        <div className="flex gap-2">
          {isCustomRoom ? (
            <input type="text" value={customRoomName} onChange={e => setCustomRoomName(e.target.value)}
              onKeyDown={e => e.key === 'Enter' && handleAddRoom()}
              placeholder="Enter room name" className="flex-1 border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500" />
          ) : (
            <select value={selectedRoom} onChange={e => setSelectedRoom(e.target.value)} className="flex-1 border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500">
          ...
          <button onClick={handleAddRoom} disabled={isCustomRoom ? !customRoomName.trim() : !selectedRoom}
            className="px-4 py-2 text-sm font-medium text-white bg-primary-600 rounded-md hover:bg-primary-700 disabled:opacity-40">
            Add Room
          </button>
        </div>
        <p className="text-xs text-gray-400 mt-2">Use "+ Add Item" inside each room to pick from the pre-set item list for that room type.</p>
      </div>

      {profile.rooms.length === 0 ? (
        <div className="bg-white rounded-lg shadow p-8 text-center text-gray-400 text-sm">No rooms added yet.</div>
          ...
          <div key={room.id} className="bg-white rounded-lg shadow overflow-hidden">
            <div className="flex items-center gap-2 px-4 py-3 bg-gray-50 border-b border-gray-200">
              <div className="flex flex-col gap-0.5">
                <button onClick={() => reorderRoom(room.id, 'up')} disabled={ri === 0} className="w-5 h-4 flex items-center justify-center text-gray-400 hover:text-gray-700 disabled:opacity-20 text-xs">▲</button>
                <button onClick={() => reorderRoom(room.id, 'down')} disabled={ri === profile.rooms.length - 1} className="w-5 h-4 flex items-center justify-center text-gray-400 hover:text-gray-700 disabled:opacity-20 text-xs">▼</button>
              </div>
              ...
              {editingRoomId === room.id ? (
                <input autoFocus type="text" value={editingRoomName} onChange={e => setEditingRoomName(e.target.value)}
                  ...
                  className="flex-1 border border-primary-400 rounded px-2 py-1 text-sm font-semibold focus:outline-none"
                />
              ) : (
                <div className="flex-1 flex items-center gap-2 cursor-pointer select-none" onClick={() => setExpandedRoom(expandedRoom === room.id ? null : room.id)}>
                  <span className="text-gray-400 text-xs">{expandedRoom === room.id ? '▼' : '▶'}</span>
                  <span
                  ...
                  >{room.name}</span>
                  <span className="text-xs text-gray-400 bg-gray-200 px-2 py-0.5 rounded-full">{room.items.length} item{room.items.length !== 1 ? 's' : ''}</span>
                </div>
              )}
              ...
              >Edit List</button>
              <button onClick={() => startEditRoom(room.id, room.name)} className="text-gray-300 hover:text-primary-500 text-xs px-1" title="Rename room">✎</button>
              <button onClick={() => { if (confirm(`Delete "${room.name}"?`)) deleteRoom(room.id); }} className="text-gray-300 hover:text-red-500 text-xl px-1">×</button>
            </div>
              ...
              <div className="p-4 bg-gray-50 space-y-2">
                ...
                {room.items.length > 0 && (
                  <div className="flex items-center gap-2 px-2 pb-1">
                    <span className="w-36 flex-shrink-0 text-xs font-medium text-gray-500">Item <span className="font-normal text-gray-400">(click to rename)</span></span>
                    <span className="flex-1 text-xs font-medium text-gray-500 min-w-0">Brand / Model</span>
                    <span className="w-14 text-xs font-medium text-gray-500 text-center flex-shrink-0">Qty</span>
                    <span className="flex-1 text-xs font-medium text-gray-500 min-w-0">Remarks</span>
                ...
                {addingTo === room.id ? (
                  ...
                  <button onClick={() => setAddingTo(room.id)} className="w-full mt-1 py-2 text-sm text-primary-600 border-2 border-dashed border-primary-200 rounded-lg hover:bg-primary-50 transition-colors">+ Add Item</button>
                )}
              </div>
            )}
          </div>
